...
function openPopup(popupContainer){
   ...
   if(currentSource !== 0){
      ...
      console.log(parseInt((currentSource-1)/3));
      popup.children[parseInt((currentSource-1)/3)].children[(currentSource-1)%3].setAttribute('style','opacity: 0.3; cursor: default');
      popup.children[parseInt((currentSource-1)/3)].children[(currentSource-1)%3].classList.remove('add-number-hover');
      popup.children[parseInt((currentSource-1)/3)].children[(currentSource-1)%3].removeAttribute('onclick');
   }
}
...
function buildChainSource(){
   ...
   if (chainField.children[0].children[0].classList.contains('chain-add')){
      chainField.children[0].setAttribute('style','display:none;');
   }
}
